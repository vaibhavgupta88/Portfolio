...
export const LeadershipCard: React.FC = () => {
  return (
    <motion.div 
      ...
      className="bg-white/80 hover:bg-white/90 backdrop-blur-2xl rounded-[28px] p-6 sm:p-7 border border-white/70 shadow-[0_8px_32px_rgba(0,0,0,0.03),inset_0_1px_0_rgba(255,255,255,0.9)] hover:shadow-[0_14px_40px_rgba(0,0,0,0.06),inset_0_1px_0_rgba(255,255,255,1)] flex flex-col justify-start h-[340px] relative overflow-hidden group transition-all duration-300 select-none cursor-default"
    >
          ...
          <div key={act.id} className="group/item cursor-default">
            <div className="transition-transform duration-200 group-hover/item:translate-x-1">
              <div className="flex items-center justify-between">
                <h4 className="text-[14.5px] font-bold text-[#111111] leading-tight">
                  {act.name}
                </h4>
              ...
              <p className="text-[10.5px] font-medium text-[#8A8A85] mt-0.5 tracking-tight truncate">
                {act.skills}
              </p>
            </div>

            {idx === 0 && (
              <div className="mt-3.5 border-b border-neutral-200/60" />
            )}
          </div>
        ))}
      </div>
